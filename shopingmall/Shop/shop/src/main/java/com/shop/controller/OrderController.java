...
/*
1 -> 스프링에서 비동기 처리를 할 때 @RequestBody와 @ResponseBody 어노테이션을 사용합니다.

RequestBody-> HTTP 요청의 본문 body에 담긴 내용을 자바 객체로 전달

ResponseBody-> 자바 객체를 HTTP 요청의 body로 전달

2-> 주문 정보를 받는 orderDto 객체에 데이터 바인딩 시 에러가 있는지 검사합니다.

3-> 에러 정보를 ResponseEntity 객체에 담아서 반환합니다.

4-> 현재 로그인 유저의 정보를 얻기 위해서 @Controller 어노테이션이 선언된 클래스에서 메소드 인자로 principal객체를 넘겨

줄 경우 해당 객체에 직접 접근할 수 있습니다. principal 객체에서 현재 로그인한 회원의 이메일 정보를 조회합니다.

5-> 화면으로 부터 넘어오는 주문정보와 회원의 이메일 정보를 이용하여 주문 로직을 호출합니다.

6-> 결과값으로 생성된 주문 번호와 요청이 성공했다는 HTTP 응답 상태코드를 반환합니다.
