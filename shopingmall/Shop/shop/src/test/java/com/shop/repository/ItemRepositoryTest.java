...
class ItemRepositoryTest {
    ...
    public void findByItemNmOrItemDetailTest(){
        ...
        List<Item> itemList = itemRepository.findByItemNmOrItemDetail("테스트 상품1", "테스트 상품 상세 설명5");//9
        for(Item item : itemList){
    ...
    public void findByDeatilByNative(){
        this.createItemList();
    ...
    public void queryDslTest(){
        this.createItemList();
        JPAQueryFactory queryFactory = new JPAQueryFactory(em);//7
        QItem qItem = QItem.item;//8
        JPAQuery<Item> query  = queryFactory.selectFrom(qItem)//9
                .where(qItem.itemSellStatus.eq(ItemSellStatus.SELL))
                .where(qItem.itemDetail.like("%" + "테스트 상품 상세 설명" + "%"))
                .orderBy(qItem.price.desc());

        List<Item> itemList = query.fetch();//10
    ...
    public void createItemList2(){//dsl주석1
        for(int i=1;i<=5;i++){
            Item item = new Item();
            item.setItemNm("테스트 상품" + i);
            item.setPrice(10000 + i);
